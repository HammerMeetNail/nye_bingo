	...
	rows, err := s.db.Query(ctx,
		`SELECT f.id, f.user_id, f.friend_id, f.status, f.created_at,
		        ...
		        CASE WHEN f.user_id = $1
		             THEN (u2.billing_plan = 'premium' AND (u2.billing_current_period_end IS NULL OR u2.billing_current_period_end > NOW()))
		             ELSE (u1.billing_plan = 'premium' AND (u1.billing_current_period_end IS NULL OR u1.billing_current_period_end > NOW()))
		        END
		 FROM friendships f
